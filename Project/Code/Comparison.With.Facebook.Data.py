...
for p in probability:
    ...
    for i in range(1,t):
        ...
        if(i % 200 == 0):
            nx.write_gpickle(G, path + "model_graph_"+str(p)+"_"+str(i)+".pickle")
...
plt.ylabel('E[$n_t$]')
plt.legend(loc='best')
plt.savefig('plots/model_nodes.png')

plt.figure(2)
plt.xlabel('t')
plt.ylabel('E[$m_t$]')
plt.legend(loc='best')
plt.savefig('plots/model_edges.png')

plt.figure(3)
plt.xlabel('t')
plt.ylabel('run-time')
plt.legend(loc='best')
plt.savefig('plots/model_runtime.png')
